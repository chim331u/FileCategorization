...
public static class FilesQueryEndpoint
{
    ...
    public static RouteGroupBuilder MapFilesQueryEndPoints(this RouteGroupBuilder group)
    {
        ...
        group.MapGet("/categories", GetCategoriesAsync)
            ...
            .WithSummary("Gets all distinct file categories")
            .WithDescription("Retrieves a list of all distinct file categories available in the system")
            .Produces<IEnumerable<string>>(StatusCodes.Status200OK)
            .Produces(StatusCodes.Status500InternalServerError);

        // Get filtered files  
        group.MapGet("/files/filtered/{filterType:int}", GetFilteredFilesAsync)
            .WithName("GetFilteredFiles")
            .WithSummary("Gets files filtered by type")
            .WithDescription("Retrieves files based on filter type: 1=All, 2=Categorized, 3=ToCategorize, 4=New")
        ...
        group.MapGet("/files/lastview", GetLastViewAsync)
            .WithName("GetLastViewFiles")
            .WithSummary("Gets the latest file from each category")
            .WithDescription("Retrieves the most recent file for each category for the last view functionality")
            .Produces<IEnumerable<FilesDetailResponse>>(StatusCodes.Status200OK)
            .Produces(StatusCodes.Status500InternalServerError);
        ...
        group.MapGet("/files/search", SearchFilesAsync)
            ...
            .WithSummary("Search files by name pattern")
            .WithDescription("Searches for files that match the provided name pattern")
            .Produces<IEnumerable<FilesDetailResponse>>(StatusCodes.Status200OK)
            .Produces(StatusCodes.Status400BadRequest)
            .Produces(StatusCodes.Status500InternalServerError);
