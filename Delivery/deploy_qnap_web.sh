...
#############################################################################
# FileCategorization WEB Deployment Script for QNAP ARM32 NAS
...
# Based on deploy_qnap_api.sh patterns - adapted for Blazor WebAssembly
#############################################################################
...
LOCAL_REPO_DIR="${LOCAL_REPO_DIR:-/tmp/FileCategorization_Web}"
...
DOCKERFILE_PATH="${DOCKERFILE_PATH:-Delivery/web.dockerfile}"
...
# API Base URL - Points to your FileCategorization API container
...
clone_repository() {
    ...
    if [[ -z "$FOUND_DOCKERFILE" ]]; then
        ...
        RECURSIVE_SEARCH=$(find . -name "web.dockerfile" -type f | head -1)
...
generate_appsettings() {
    ...
    local appsettings_file="$LOCAL_REPO_DIR/FileCategorization_Web/wwwroot/appsettings.json"
...
print_banner() {
    echo "============================================================================="
    echo "  FileCategorization WEB - QNAP ARM32 NAS Deployment Script"
    echo "============================================================================="
...
DESCRIPTION:
    This script performs complete deployment of FileCategorization Web 
...
QUICK START:
    # 1. Edit configuration section in the script (lines 45-85)
    # 2. Run deployment
    $0
...
OPTIONS:
    ...
    -p, --port PORT         Host port for Web (default: 80)
    --https-port PORT       Host HTTPS port (default: 443)
    -n, --name NAME         Container name (default: filecat_web)
    -i, --image NAME        Docker image name (default: filecat_web_image)
    --api-url URL           API base URL (default: http://localhost:30219/)

CONFIGURATION:
    All parameters are configured in the script header (lines 45-85):
    
    # Main Configuration
    GITHUB_REPO             Git repository URL
    GIT_BRANCH              Git branch to deploy (default: DeliveryNasArm32)
    HOST_PORT               Host port for Web (default: 80)
    HOST_HTTPS_PORT         Host HTTPS port (default: 443)
    CONTAINER_NAME          Docker container name (default: filecat_web)
    
    # API Integration
    API_BASE_URL            FileCategorization API URL (REQUIRED)
...
ENVIRONMENT VARIABLE OVERRIDE:
    ...
    export API_BASE_URL="http://your-nas-ip:30219/"
...
INTEGRATION:
    This Web deployment is designed to work with FileCategorization API.
    Make sure to:
    1. Deploy FileCategorization API first (using deploy_qnap_api.sh)
...
main() {
    ...
    print_summary
    success "FileCategorization Web deployed successfully on QNAP ARM32 NAS!"
}
